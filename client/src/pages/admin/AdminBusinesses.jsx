  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 w-full max-w-md">
        <h2 className="text-white font-semibold text-lg mb-5">Editar negocio</h2>
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-white text-2xl font-bold">Negocios <span className="text-white/40 text-base font-normal">({total})</span></h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-white/40 text-left border-b border-white/10">
                ...
                <th className="pb-3 pr-4 font-medium">Industria</th>
            ...
            <tbody>
              {items.map((u) => (
                <tr key={u.id} className="border-b border-white/[0.05] hover:bg-white/[0.03] transition">
                  <td className="py-3 pr-4 text-white">{u.businessProfile?.companyName || u.name}</td>
                  <td className="py-3 pr-4 text-white/60">{u.email}</td>
                  <td className="py-3 pr-4 text-white/60">{u.businessProfile?.industry || '—'}</td>
                  <td className="py-3 pr-4 text-white/60">{u.city || '—'}</td>
          ...
          {items.length === 0 && (
            <p className="text-white/40 text-center py-12">No hay negocios registrados</p>
          )}
        </div>
      )}
